...
import os      #os is defined library
...
# to insert emojis, inside "press windows button on keyboard+ ."
# to run streamlit: write command in terminal: streamlit run .\app.py
...
if tab1:
    if st.button("Recommend Diet"):
        if client:
            with st.spinner("Creating Diet..."):      #AI shows output in each line for clarity
                try:
                     db=load_rag()
                     search_query=f"""diet_type{diet_type}
                                        ...
                                        Protein
                                        Allergies {allergies}"""
                     docs=db.similarity_search(search_query,3) # shows similar searches of what we have written. Max 3 searches can be done or we specify
...
if tab2:
    ...
    #text area to get input from user. Multi lines, therefore area
    if st.button("Ask AI"):
        db=load_rag()
